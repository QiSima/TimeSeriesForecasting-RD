...
from models.seq2seq.Attention.modules.masking import TriangularCausalMask, ProbMask
...
class VariantFusionAttention(nn.Module):
    def __init__(self, out_dim, n_units, inp_dim):
        super(FusionAttention, self).__init__()
        self.func_q = nn.Linear(2 * n_units, 1)
        self.query_projection = nn.Linear(inp_dim, 2 * n_units)   
        # self.func_g = nn.Linear(2 * n_units, 1)

    def forward(self, hidden_state, p, input):
        '''
            hidden_state : [B, N, D]
            p : [B, N, D]
            input : [B, out_dim, inp_dim] (inp_dim including : pos & values)
        '''
        # concate    [B, N , 2 * D]
        new_states = torch.cat((hidden_state, p),dim=2)
        # calculate u  [B, N, 1]
        mu = self.func_q(new_states)
        # queries: [B, out_dim, 2 * n_units]
        queries = self.query_projection(input)

        # calculate s    [B, out_dim, 2 * n_units], [B, N , 2*D] ——>[B, N ,out_dim]    (torch.einsum("qd,bnd->bnq", v, new_states))
        scores = torch.einsum("bod,bnd->bno", queries, new_states)
        s = torch.exp(scores)
        s = s / torch.sum(s,dim=1,keepdim=True)
        s = 1e-2 * -torch.log2(s)
